...
#用于裁剪掩码（masks）以适应给定的边界框
def crop_mask(masks, boxes):
    """
    It takes a mask and a bounding box, and returns a mask that is cropped to the bounding box.
    这个 crop_mask 函数的作用是将一组掩码（masks）裁剪到指定的边界框（bounding boxes）内。
    掩码通常用于表示图像中某些对象的像素级位置，而边界框则定义了这些对象在图像中的位置和大小。
    这个函数可以用于确保掩码只包含在边界框内的像素，从而去除边界框外的无效区域。
    Args:
